...
function simplePdf(devis, entreprise) {
  ...
  const lines = [
    ...
    ...(devis.lignes || []).map(
      (l) =>
        `${l.designation} | Qte ${l.quantite} | ${money(l.montantTtc, currency)}`,
    ),
...
export function buildDevisHtml(devis, entreprise = {}) {
  ...
  const rows = (devis.lignes || [])
    .map(
      (ligne, index) =>
        `<tr><td>${index + 1}</td><td>${escapeHtml(ligne.produit?.reference || "-")}</td><td>${escapeHtml(ligne.designation)}</td><td class="num">${ligne.quantite}</td><td class="num">${money(ligne.prixUnitaireHt, currency)}</td><td class="num">${Number(ligne.remise || 0)} %</td><td class="num">${Number(ligne.tauxTva || 0)} %</td><td class="num">${money(ligne.montantTtc, currency)}</td></tr>`,
    )
    .join("");
...
export async function buildDevisPdf(devis, entreprise) {
  try {
    const { buffer } = await renderPdfDocument({
      html: buildDevisHtml(devis, entreprise),
      pdfOptions: {
        format: "A4",
