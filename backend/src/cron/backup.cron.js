...
export function startBackupCron() {
  ...
  cron.schedule(
    ...
    async () => {
      ...
      try {
        ...
        logger.info(
          `CRON backup OK : ${result.backup.filename} (${result.backup.sizeKb} Ko) | ${result.deletedOldBackups} ancien(s) supprimé(s)`,
        );
      } catch (error) {
  ...
  logger.info("CRON backup planifié (chaque jour à 16h)");
}
