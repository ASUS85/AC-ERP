...
export const produitsService = {
  async list(query) {
    ...
    const where = {
      ...(query.search ? { OR: [{ designation: { contains: query.search } }, { reference: { contains: query.search } }] } : {}),
      ...(query.categorieId ? { idCategorie: query.categorieId } : {}),
      ...(query.statut ? { statut: query.statut } : {}),
    };
    const [data, total] = await Promise.all([
      produitsRepository.findMany({ where, skip: offset, take: limit, orderBy: { createdAt: "desc" } }),
  ...
  async create(data, context) {
    ...
    return produitsRepository.createWithStock({ ...payload, reference }, Number(stockInitial), context.user?.userId);
  },
  async update(id, data) { await this.getById(id); return produitsRepository.update(id, data); },
  async remove(id) {
    const produit = await this.getById(id);
    if ((produit.stock?.stockActuel || 0) > 0) throw new ApiError(400, "STOCK_NOT_EMPTY", "Impossible d'archiver un produit avec stock");
