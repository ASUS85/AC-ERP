...
async function main() {
  ...
  const roles = await Promise.all([
    prisma.role.upsert({
      ...
      update: {},
      create: { nomRole: 'SUPER_ADMIN', description: 'Accès total au système', isSystemRole: true }
    }),
    prisma.role.upsert({
      where: { nomRole: 'ADMIN' },
      update: {},
      create: { nomRole: 'ADMIN', description: 'Gestion des données métier', isSystemRole: true }
    }),
    prisma.role.upsert({
      where: { nomRole: 'COMMERCIAL' },
      update: {},
      create: { nomRole: 'COMMERCIAL', description: 'Gestion des ventes et clients', isSystemRole: true }
    }),
    prisma.role.upsert({
      where: { nomRole: 'ACHETEUR' },
      update: {},
      create: { nomRole: 'ACHETEUR', description: 'Gestion des achats', isSystemRole: true }
    }),
    prisma.role.upsert({
      where: { nomRole: 'GESTIONNAIRE_STOCK' },
      update: {},
      create: { nomRole: 'GESTIONNAIRE_STOCK', description: 'Gestion des stocks', isSystemRole: true }
    }),
    prisma.role.upsert({
      where: { nomRole: 'COMPTABLE' },
      update: {},
      create: { nomRole: 'COMPTABLE', description: 'Gestion financière', isSystemRole: true }
    }),
    prisma.role.upsert({
      where: { nomRole: 'DIRECTEUR' },
      update: {},
      create: { nomRole: 'DIRECTEUR', description: 'Consultation tableau de bord', isSystemRole: true }
    }),
  ]);
  ...
  const permissionByKey = new Map(permissions.map((permission) => [`${permission.module}:${permission.action}`, permission]));
  const roleByName = new Map(roles.map((role) => [role.nomRole, role]));

  const rolePermissions = {
    SUPER_ADMIN: permissionsData.map((permission) => `${permission.module}:${permission.action}`),
    ADMIN: permissionsData.map((permission) => `${permission.module}:${permission.action}`),
    COMMERCIAL: [
      'dashboard:lire',
      'categories:lire',
      'produits:lire',
  ...
  const superAdminRole = await prisma.role.findUnique({ where: { nomRole: 'SUPER_ADMIN' } });
  ...
  console.log('✅ Compte admin créé : armandchristian85@gmail.com / Admin@1234');
  ...
  console.log(`✅ ${usersData.length} utilisateurs métiers créés : mot de passe ${defaultUserPassword}`);
