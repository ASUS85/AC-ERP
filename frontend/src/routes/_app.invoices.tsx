...
const cols: Column<I>[] = [
  { key: "ref", header: "N° facture", render: (i) => <span className="font-medium text-foreground">{i.ref}</span> },
  { key: "client", header: "Client" },
  { key: "echeance", header: "Échéance" },
  { key: "montant", header: "Montant", align: "right", render: (i) => <span className="font-medium text-foreground">{fmtCurrency(i.montant)}</span> },
  { key: "statut", header: "Statut", align: "right", render: (i) => <StatusBadge status={i.statut} /> },
];
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Button size="sm" className="gap-1.5" onClick={() => toast.info("Nouvelle facture")}>
        ...
        <SectionCard
          ...
          action={
            <div className="flex gap-1.5">
              <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => toast.info("Impression…")}>
                <Printer className="h-4 w-4" />
              </Button>
              <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => toast.success("PDF exporté")}>
          ...
          <div className="rounded-lg border border-border bg-card p-5 text-sm">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="font-display font-bold text-foreground">AC ERP</p>
                  <p className="text-xs text-muted-foreground">12 rue du Commerce, Lyon</p>
                </div>
              </div>
            ...
            <div className="my-4 border-t border-border pt-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">Facturé à : TechnoPlus SARL</p>
              ...
              <tbody>
                {lines.map((l) => (
                  <tr key={l.d} className="border-b border-border/60">
                    ...
                    <td className="py-2 text-center text-muted-foreground">{l.q}</td>
                    <td className="py-2 text-right text-foreground">{fmtCurrency(l.q * l.pu)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <div className="mt-3 space-y-1 text-xs">
              <div className="flex justify-between text-muted-foreground"><span>Total HT</span><span>{fmtCurrency(ht)}</span></div>
              <div className="flex justify-between text-muted-foreground"><span>TVA 20 %</span><span>{fmtCurrency(tva)}</span></div>
              <div className="flex justify-between border-t border-border pt-1 font-bold text-foreground"><span>Total TTC</span><span>{fmtCurrency(ht + tva)}</span></div>
            </div>
          </div>
        </SectionCard>
      </div>
