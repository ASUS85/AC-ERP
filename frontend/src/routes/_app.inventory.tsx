...
import { Warehouse, ArrowDownToLine, ArrowUpFromLine, AlertTriangle, ClipboardList } from "lucide-react";
...
const cols: Column<M>[] = [
  { key: "ref", header: "Référence", render: (m) => <span className="font-medium text-foreground">{m.ref}</span> },
  ...
  { key: "type", header: "Type", render: (m) => <StatusBadge status={m.type} /> },
  {
    ...
    header: "Quantité",
    align: "right",
    render: (m) => <span className={cn("font-medium", m.type === "Entrée" ? "text-success" : "text-info")}>{m.type === "Entrée" ? "+" : "−"}{m.qte}</span>,
  },
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <Button variant="outline" size="sm" className="gap-1.5" onClick={() => toast.info("Nouvelle entrée de stock")}>
              <ArrowDownToLine className="h-4 w-4" /> Entrée
            </Button>
            <Button size="sm" className="gap-1.5" onClick={() => toast.info("Nouvel inventaire")}>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <SectionCard title="Mouvements de stock" description="Entrées et sorties récentes" className="lg:col-span-2">
          <div className="mb-4">
            <Toolbar placeholder="Rechercher un mouvement…" />
          </div>
          <DataTable columns={cols} rows={stockMovements} rowKey={(m) => m.ref} withActions={false} />
        </SectionCard>
        <SectionCard title="Alertes de rupture" description="Produits à réapprovisionner">
          <div className="space-y-3">
            {lowStock.map((p) => (
              <div key={p.ref} className="flex items-center justify-between gap-2 rounded-lg border border-border p-3">
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium text-foreground">{p.nom}</p>
                  <p className="text-xs text-muted-foreground">Stock : {p.stock} unités</p>
                </div>
