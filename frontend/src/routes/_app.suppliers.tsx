...
const cols: Column<S>[] = [
  ...
  { key: "total", header: "Total achats", align: "right", render: (s) => <span className="font-medium text-foreground">{fmtCurrency(s.total)}</span> },
  { key: "statut", header: "Statut", align: "right", render: (s) => <StatusBadge status={s.statut} /> },
];

function SuppliersPage() {
  return (
    <>
      <PageHeader title="Fournisseurs" description="Coordonnées, commandes et statistiques" breadcrumb={["Gestion commerciale", "Fournisseurs"]} />
      ...
      <SectionCard title="Liste des fournisseurs">
        <div className="mb-4">
          <Toolbar placeholder="Rechercher un fournisseur…" addLabel="Ajouter un fournisseur" onAdd={() => toast.info("Ajout d'un fournisseur")} />
        </div>
