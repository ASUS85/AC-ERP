...
function SalesPage() {
  const [cart, setCart] = useState<
    { ref: string; nom: string; prix: number; qte: number }[]
  >([
  ...
  const add = (p: (typeof products)[number]) =>
    setCart((c) => {
      ...
      if (ex)
        return c.map((i) => (i.ref === p.ref ? { ...i, qte: i.qte + 1 } : i));
      return [...c, { ref: p.ref, nom: p.nom, prix: p.prix, qte: 1 }];
    });
  const setQte = (ref: string, d: number) =>
    setCart((c) =>
      c.map((i) => (i.ref === ref ? { ...i, qte: Math.max(1, i.qte + d) } : i)),
    );
  ...
  return (
    <>
      <PageHeader
        title="Ventes"
        description="Création de ventes, panier et facturation"
