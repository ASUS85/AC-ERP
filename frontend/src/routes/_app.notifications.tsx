...
import { AlertTriangle, FileWarning, Sparkles, CheckCircle2, CheckCheck, LoaderCircle } from "lucide-react";
import { PageHeader } from "@/components/erp/PageHeader";
import { SectionCard } from "@/components/erp/widgets";
import { Button } from "@/components/ui/button";
import { Tabs, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { getNotifications, marquerLue, marquerToutesLues } from "@/lib/api/notifications.service";
...
const isAiNotification = (notification: Notification) =>
  ...
  /\b(ia|intelligence artificielle|prévision)\b/i.test(`${notification.titre} ${notification.message}`);
...
function NotificationsPage() {
  ...
  const toggleNotification = async (notification: Notification) => {
    ...
    if (!notification.isLue) {
      try {
        ...
        setItems((current) => current.map((item) => item.id === notification.id ? { ...item, isLue: true } : item));
      } catch {
  ...
  return (
    <>
      ...
      <SectionCard title="Activité" description={`${visibleItems.length} notification${visibleItems.length > 1 ? "s" : ""} pour cette date`}>
        <div className="space-y-2">
          {loading && <div className="flex justify-center py-8"><LoaderCircle className="h-5 w-5 animate-spin text-muted-foreground" /></div>}
          {!loading && visibleItems.length === 0 && <p className="py-8 text-center text-sm text-muted-foreground">Aucune notification pour cette date.</p>}
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  n.isLue ? "border-border" : "border-primary/20 bg-primary/[0.03]",
                )}
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {!n.isLue && <span className="h-1.5 w-1.5 rounded-full bg-primary" />}
                  </div>
                  <p className={cn("whitespace-pre-wrap break-words text-sm text-muted-foreground", !expanded && "line-clamp-2")}>{n.message}</p>
                  <p className="mt-0.5 text-xs text-muted-foreground/70">{relativeTime(n.createdAt)}</p>
                </div>
              </button>
            );
          })}
