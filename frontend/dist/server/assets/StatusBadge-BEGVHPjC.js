...
const map = {
  ...
  emise: "bg-info/12 text-info border-info/20",
  "partiellement payee": "bg-warning/15 text-warning-foreground border-warning/30",
  soldee: "bg-success/12 text-success border-success/20",
  soumis: "bg-info/12 text-info border-info/20",
  envoye: "bg-info/12 text-info border-info/20",
  confirme: "bg-success/12 text-success border-success/20",
...
function StatusBadge({ status }) {
  const key = status?.trim().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
