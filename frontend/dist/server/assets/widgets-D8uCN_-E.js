...
function SearchableSelect({
  ...
  const updateDropdownPosition = React.useCallback(() => {
    ...
    const shouldOpenUp = availableBelow < dropdownHeight + dropdownGap && availableAbove > availableBelow;
    setDropdownStyle({
      position: "fixed",
      left: rect.left,
      top: shouldOpenUp ? Math.max(8, rect.top - dropdownGap - dropdownHeight) : rect.bottom + dropdownGap,
      width: rect.width,
      maxHeight: Math.max(
        120,
        Math.min(
